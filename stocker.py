...
Fifty_Fifty = _predefined_portfolio(name="All Bonds",
                                    positions=[US_Stocks(), International_Stocks(), US_Bonds(), International_Bonds()],
                                    weights=[7, 3, 7, 3])
# 60/40 Stocks and bonds with 70% US and 30% International
Sixty_Forty = _predefined_portfolio(name="All Bonds",
                                    positions=[US_Stocks(), International_Stocks(), US_Bonds(), International_Bonds()],
                                    weights=[7 * 6, 3 * 6, 7 * 4, 3 * 4])
...
class _Scenario_Base(metaclass=abc.ABCMeta):
    ...
    def results(self):
        ...
        # strn += "\n"
        strn += self.portfolio.name + " Portfolio End (Inflation Corrected at " + _format_percentage(
            self.inflation_rate) + "):\n"
        strn += str(self.history[-1])
        strn += "\n"
        strn += "Raw Metrics:\n"
        strn += "Ave Return: " + _format_percentage(
            sum(self.uncorrected_returns) / len(self.uncorrected_returns)) + "\n"
        strn += "Best Return: " + _format_percentage(max(self.uncorrected_returns)) + " (year " + str(
            self.returns.index(max(self.returns))) + ")\n"
        strn += "Worst Return: " + _format_percentage(min(self.uncorrected_returns)) + " (year " + str(
            self.returns.index(min(self.returns))) + ")\n"
        strn += "\n"
        ...
        strn += "Best Return: " + _format_percentage(max(self.returns)) + " (year " + str(
            self.returns.index(max(self.returns))) + ")\n"
        strn += "Worst Return: " + _format_percentage(min(self.returns)) + " (year " + str(
            self.returns.index(min(self.returns))) + ")\n"
        return strn
...
class Scenario(_Scenario_Base):
    def __init__(self, name, portfolio, num_years, inflation_rate_perc=3.5, rebalance=True, annual_contribution=0.0,
                 annual_contribution_increase_perc=0.0, end_weights=None):
        ...
        if self.end_weights:
            assert len(end_weights) == len(
                portfolio.weights), "Length of end weight vector and length of start weights in portfolio must be equal."
            self.slopes = [float(w_end - w_start) / float(num_years - 1) for w_end, w_start in
                           zip(self.end_weights, portfolio.weights)]
...
class Monte_Carlo(object):
    ...
    def plot(self, smooth=False):
        ...
        i125 = round(total_trials * 0.125)
        i025 = round(total_trials * 0.025)

        import matplotlib.pyplot as plt
        from scipy.signal import savgol_filter

        # Find the data sets from self (scenario):
        # Find the median and 10th percentile data sets:
...
#                print("result of year "+str(year)+" in run "+ str(run) + " was "+str(some_year_result))
        ...
        scenario_875 = self.runs[self.raw_values.index(p875)]
        scenario_750 = self.runs[self.raw_values.index(p750)]
        scenario_500 = self.runs[self.raw_values.index(p500)]
...
#        data = [p.value() / 1000000.0 for p in med_scenario.history]
        ...
        y75 = [p.value()/1000000 for p in scenario_750.history]
